...
function buildDetails(type: string, payload: Record<string, unknown> | null): {
  title: string
  body?: string
...
export default async function NotificationsPage() {
  ...
  const [rows, userRows] = await Promise.all([
    db
      ...
      .limit(50),
    db
      .select({ email: users.email, phone: users.phone, notifyEmail: users.notifyEmail, notifySms: users.notifySms })
      .from(users)
