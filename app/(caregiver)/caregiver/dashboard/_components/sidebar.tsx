...
const NAV_LINKS = [
  ...
  { href: '/caregiver/dashboard/messages',   label: 'Messages' },
]
  ...
  const navLinks = (
    <nav className="space-y-1 px-3">
      {NAV_LINKS.map((link) => {
        ...
        const isMessages = link.href === '/caregiver/dashboard/messages'
      ...
      <div className={`lg:hidden fixed top-14 left-0 right-0 z-40 bg-card border-b border-border shadow-lg overflow-hidden transition-all duration-200 ${open ? 'max-h-[calc(100vh-3.5rem)] overflow-y-auto' : 'max-h-0'}`}>
        ...
        <div className="border-t border-border px-4 py-4 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => router.push('/caregiver/dashboard/profile')} className="p-1.5 rounded-md hover:bg-accent">
              <User className="h-4 w-4 text-muted-foreground" />
            </button>
            <button onClick={() => router.push('/caregiver/dashboard/notifications')} className="relative p-1.5 rounded-md hover:bg-accent">
