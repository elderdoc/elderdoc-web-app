...
import {
  Bell, LogOut, Menu, User, X, Home, Search, Inbox, Briefcase, Clock,
  ClipboardList, Calendar, Wallet, MessageSquare,
} from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
...
const NAV_LINKS = [
  { href: '/caregiver/dashboard',            label: 'Home',       icon: Home },
  { href: '/caregiver/dashboard/find-jobs',  label: 'Find Jobs',  icon: Search },
  { href: '/caregiver/dashboard/offers',     label: 'Offers',     icon: Inbox },
  { href: '/caregiver/dashboard/my-jobs',    label: 'My Jobs',    icon: Briefcase },
  { href: '/caregiver/dashboard/shifts',     label: 'Shifts',     icon: Clock },
  { href: '/caregiver/dashboard/care-plans', label: 'Care Plans', icon: ClipboardList },
  { href: '/caregiver/dashboard/calendar',   label: 'Calendar',   icon: Calendar },
  { href: '/caregiver/dashboard/payouts',    label: 'Payouts',    icon: Wallet },
  { href: '/caregiver/dashboard/messages',   label: 'Messages',   icon: MessageSquare },
]
  ...
  return (
    <>
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 flex items-center justify-between border-b border-border bg-background/95 backdrop-blur-md px-5 h-14">
        <Link href="/caregiver/dashboard" className="text-[18px] font-semibold tracking-tight">
          Elderdoc
        </Link>
        <button
          onClick={() => setOpen(v => !v)}
      ...
      <div className={`lg:hidden fixed top-14 left-0 right-0 z-40 bg-background border-b border-border shadow-[0_24px_60px_-12px_rgba(15,20,16,0.12)] overflow-hidden transition-all duration-300 ${open ? 'max-h-[calc(100vh-3.5rem)] overflow-y-auto' : 'max-h-0'}`}>
        ...
        <div className="border-t border-border px-4 py-4 flex items-center gap-3 bg-[var(--cream-deep)]/40">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => router.push('/caregiver/dashboard/profile')} className="p-2 rounded-full hover:bg-muted">
              <User className="h-4 w-4 text-foreground/70" />
            </button>
            <button onClick={() => router.push('/caregiver/dashboard/notifications')} className="p-2 rounded-full hover:bg-muted">
      ...
      <aside className="hidden lg:flex h-screen w-[260px] shrink-0 flex-col border-r border-border bg-card">
        <div className="px-6 pt-6 pb-4 border-b border-border">
          <Link href="/caregiver/dashboard" className="text-[22px] font-semibold tracking-tight">
            Elderdoc
          </Link>
          <div className="mt-1 text-[12px] text-muted-foreground">Caregiver dashboard</div>
        </div>
