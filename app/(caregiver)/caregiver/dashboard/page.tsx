...
function MatchedJobCard({ job, rank, careTypeLabels }: { job: MatchedJob; rank: number; careTypeLabels: Record<string, string> }) {
  const careTypeLabel = careTypeLabels[job.careType] ?? job.careType
...
async function MatchedJobsSection({ profileId, careTypeLabels }: { profileId: string; careTypeLabels: Record<string, string> }) {
    ...
    <section className="mb-10">
      ...
      <div className="space-y-3">
        {matched.slice(0, 3).map((job, i) => (
          <MatchedJobCard key={job.requestId} job={job} rank={i + 1} careTypeLabels={careTypeLabels} />
        ))}
      </div>
    </section>
  )
...
export default async function CaregiverDashboard() {
  ...
  const careTypeLabels = Object.fromEntries(careTypes.map(c => [c.key, c.label]))
  ...
  return (
    <div className="relative px-6 lg:px-10 py-8 lg:py-10 max-w-[1200px] mx-auto">
      ...
      <Suspense fallback={
        ...
        <MatchedJobsSection profileId={profile.id} careTypeLabels={careTypeLabels} />
      </Suspense>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[14px] leading-snug">
                      {item.type === 'application'
                        ? <>Applied for <span className="font-medium">{careTypeLabels[item.careType] ?? item.careType}</span> care</>
