...
export default async function ClientProfilePage() {
  ...
  const [userRows, locationRows] = await Promise.all([
    db
      .select({ id: users.id, name: users.name, email: users.email, phone: users.phone, image: users.image, role: users.role, about: users.about, notifyEmail: users.notifyEmail, notifySms: users.notifySms, createdAt: users.createdAt })
      .from(users)
