...
export function CareRequestCard({ req, careTypes }: { req: CareRequestCardData; careTypes: CareType[] }) {
  const status = req.status ?? 'draft'
  const careTypeLabel = careTypes.find((c) => c.key === req.careType)?.label ?? req.careType
