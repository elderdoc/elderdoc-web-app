...
interface Props {
  ...
  careTypeLabels: Record<string, string>
}
...
function MyCaregiverCardItem({ cg, careTypeLabels }: { cg: MyCaregiverCard; careTypeLabels: Record<string, string> }) {
...
function CardGrid({ items, careTypeLabels }: { items: MyCaregiverCard[]; careTypeLabels: Record<string, string> }) {
  if (items.length === 0) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {items.map((cg) => (
        <MyCaregiverCardItem key={cg.caregiverId} cg={cg} careTypeLabels={careTypeLabels} />
      ))}
    </div>
  )
}

export function MyCaregiversTabs({ activeTab, current, favorites, careTypeLabels }: Props) {
  return (
    <div>
      ...
      {activeTab === 'current' ? <CardGrid items={current} careTypeLabels={careTypeLabels} /> : <CardGrid items={favorites} careTypeLabels={careTypeLabels} />}
    </div>
  )
}
