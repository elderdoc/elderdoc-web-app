...
export default async function ClientMessagesPage() {
  ...
  const CARE_TYPE_LABELS = Object.fromEntries(careTypes.map(c => [c.key, c.label]))
  ...
  const [activeJobs, unreadRows] = await Promise.all([
    ...
    db
      ...
      .where(and(eq(jobs.clientId, clientId), ne(messages.senderId, clientId), eq(messages.read, false)))
    ...
    <div className="p-4 lg:p-8">
      ...
      {activeJobs.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-20 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Messages become available once a caregiver is hired.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {activeJobs.map(job => {
            const initials = (job.caregiverName ?? '?').charAt(0).toUpperCase()
            ...
            return (
              ...
              >
                {job.caregiverImage ? (
                  <img src={job.caregiverImage} alt={job.caregiverName ?? ''} className="h-10 w-10 rounded-full object-cover shrink-0" />
                ) : (
