...
async function CaregiverResults({ searchParams, isAuthenticated }: {
  ...
  const [caregivers, activeCareTypes] = await Promise.all([
    searchCaregivers({ careTypes, state: params.state }),
    getActiveCareTypes(),
  ])
  const careTypeLabels = Object.fromEntries(activeCareTypes.map(ct => [ct.key, ct.label]))
  ...
  return <PreviewClient caregivers={withDistance} isAuthenticated={isAuthenticated} searchParams={params} careTypeLabels={careTypeLabels} />
}
