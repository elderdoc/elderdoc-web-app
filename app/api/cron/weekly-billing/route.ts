...
export async function POST(req: NextRequest) {
  ...
  // Notify clients with completed unbilled shifts that have no rate set
  const noRateRows = await db
    .select({
      jobId:    shifts.jobId,
    ...
    .where(
      and(
        ...
        isNull(careRequests.budgetMin),
      )
    )

  const noRateJobsSeen = new Set<string>()
  for (const row of noRateRows) {
    if (noRateJobsSeen.has(row.jobId)) continue
    noRateJobsSeen.add(row.jobId)
    await db.insert(notifications).values({
      userId: row.clientId,
      type: 'billing_no_rate',
      payload: { jobId: row.jobId, careType: row.careType },
    })
  }
