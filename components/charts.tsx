...
export function Donut({ segments, size = 180, thickness = 22, centerLabel, centerValue, className }: DonutProps) {
  ...
  const computed = segments.reduce<{ len: number; dasharray: string; dashoffset: number }[]>((acc, s) => {
    const cumOffset = acc.reduce((sum, x) => sum + x.len, 0)
    const len = (s.value / total) * circumference
    acc.push({ len, dasharray: `${len} ${circumference - len}`, dashoffset: -cumOffset })
    return acc
  }, [])
    ...
    <div className={className}>
      <div className="relative" style={{ width: size, height: size }}>
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ transform: 'rotate(-90deg)' }}>
          ...
          {segments.map((s, i) => {
            const { dasharray, dashoffset } = computed[i]
            return (
              <circle
                key={i}
