...
const mockCareTypeLabels: Record<string, string> = {
...
describe('CaregiverCard', () => {
  it('renders caregiver name', () => {
    render(<CaregiverCard caregiver={mockCaregiver} careTypeLabels={mockCareTypeLabels} />)
  ...
  it('renders distance when distanceMiles provided', () => {
    render(<CaregiverCard caregiver={{ ...mockCaregiver, distanceMiles: 12.7 }} careTypeLabels={mockCareTypeLabels} />)
    expect(screen.getByText('📍 13 mi away')).toBeDefined()
  })

  it('renders <1 mi away for sub-mile distances', () => {
    render(<CaregiverCard caregiver={{ ...mockCaregiver, distanceMiles: 0.4 }} careTypeLabels={mockCareTypeLabels} />)
  ...
  it('calls onSendOffer when button clicked', () => {
    ...
    render(<CaregiverCard caregiver={mockCaregiver} onSendOffer={onSendOffer} careTypeLabels={mockCareTypeLabels} />)
  ...
  it('shows fallback name when name is null', () => {
    render(<CaregiverCard caregiver={{ ...mockCaregiver, name: null }} careTypeLabels={mockCareTypeLabels} />)
    expect(screen.getByText('Anonymous Caregiver')).toBeDefined()
  })

  it('shows single initial for single-word name', () => {
    render(<CaregiverCard caregiver={{ ...mockCaregiver, name: 'Madonna' }} careTypeLabels={mockCareTypeLabels} />)
