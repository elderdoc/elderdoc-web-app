  ...
  for (let i = 0; i < CLIENTS.length; i++) {
    ...
    for (const req of reqs) {
      const [request] = await db
        .insert(careRequests)
        .values({ clientId: clientUser.id, recipientId: primaryRecipientId, careType: req.careType, title: req.title, frequency: req.frequency, shifts: req.shifts, days: req.days, durationHours: req.durationHours, budgetType: req.budgetType, budgetAmount: req.budgetAmount, status: 'active', languagePref: [] })
